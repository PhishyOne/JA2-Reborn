...
int main(int argc, char* argv[])
{
    try {
		...
		if (!CheatSystem::loadFromFile(configFolderPath.get()))
		{
			SLOGD("No cheats.json loaded (file missing or unparseable); using defaults.");
		}
...
void TerminationHandler()
{
    ...
    jclass exceptionContainer = jniEnv->FindClass("com/ja2/reborn/NativeExceptionContainer");
    jfieldID singletonFieldId = jniEnv->GetStaticFieldID(exceptionContainer, "INSTANCE", "Lcom/ja2/reborn/NativeExceptionContainer;");
