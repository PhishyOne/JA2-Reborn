...
std::atomic_bool fBuffersNeedService{ false };
BOOLEAN fShutdownBufferServiceThread = FALSE;

static void RequestBufferService()
{
	fBuffersNeedService.store(true, std::memory_order_release);
...
static BOOLEAN DoesChannelRingBufferNeedService(SOUNDTAG* channel) {
	...
	auto framesToWrite = ma_pcm_rb_available_write(channel->pRingBuffer);
	// Keep Android's audio thread well ahead of loading spikes during screen transitions.
	return framesToWrite >= SOUND_SAMPLES * 4;
}
...
static int SoundServiceBuffers(void *_ptr)
{
	...
	while (1) {
		...
		conditionBuffersNeedService.wait_for(lk, std::chrono::milliseconds(25), []{
			return fBuffersNeedService.load(std::memory_order_acquire) || fShutdownBufferServiceThread;
		});
		if (fShutdownBufferServiceThread) {
			SLOGD("Stopped SoundManBufferServiceThread");
			return 0;
		}
		fBuffersNeedService.store(false, std::memory_order_release);
		lk.unlock();
		...
		switch (Sound->State)
		{
			...
			case CHANNEL_PLAY:
			{
				...
				if (samples < want_samples || rbResult == MA_AT_END) {
					if (Sound->DoneServicing) {
						Sound->State = CHANNEL_DEAD;
					} else {
						ringBuffersNeedService = TRUE;
					}
				}
