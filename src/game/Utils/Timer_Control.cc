...
static std::array<milliseconds, NUMTIMERS> giTimerIntervals
{
...
void SetScrollSpeed(int const ms)
{
	giTimerIntervals[NEXTSCROLL] = std::chrono::milliseconds{ms};
}

int GetScrollSpeed()
{
	return static_cast<int>(giTimerIntervals[NEXTSCROLL].count());
}
