...
BOOLEAN EnoughPoints(const SOLDIERTYPE* pSoldier, INT16 sAPCost, INT16 sBPCost, BOOLEAN fDisplayMsg)
{
	...
	if (CheatSystem::unlimitedAP() && CheatSystem::appliesToPlayerSoldier(pSoldier))
	{
		return TRUE;
	}
...
void DeductPoints( SOLDIERTYPE *pSoldier, INT16 sAPCost, INT16 sBPCost )
{
	...
	if (sAPCost > 0 && CheatSystem::unlimitedAP() && CheatSystem::appliesToPlayerSoldier(pSoldier))
	{
		sAPCost = 0;
	}
	if (sBPCost > 0 && CheatSystem::unlimitedBreath() && CheatSystem::appliesToPlayerSoldier(pSoldier))
	{
		sBPCost = 0;
	}
...
{
	if (CheatSystem::unlimitedAmmo() && CheatSystem::appliesToPlayerSoldier(pSoldier))
	{
		return;
	}
