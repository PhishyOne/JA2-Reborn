...
static SOLDIERTYPE* GetTeamPanelMercPortraitAt(INT16 const x, INT16 const y)
{
	if (guiCurrentScreen != GAME_SCREEN) return NULL;
	if (gsCurInterfacePanel != TEAM_PANEL) return NULL;
	if (fInMapMode || gfInItemPickupMenu || gpItemPointer != NULL) return NULL;
	...
	for (TeamPanelSlot& tp : gTeamPanel)
	{
		INT32 const face_x = dx + TM_FACE_X;
		INT32 const face_y = dy + TM_FACE_Y;
		if (x >= face_x && x < face_x + TM_FACE_WIDTH &&
			y >= face_y && y < face_y + TM_FACE_HEIGHT)
		{
			SOLDIERTYPE* const s = tp.merc;
			if (!s || !s->bActive) return NULL;
			return s;
		}
		dx += TM_INV_HAND_SEP;
	}
...
BOOLEAN IsTeamPanelMercPortraitAt(INT16 const x, INT16 const y)
{
	return GetTeamPanelMercPortraitAt(x, y) != NULL;
}


BOOLEAN HandleTeamPanelMercPortraitTouch(INT16 const x, INT16 const y)
{
	SOLDIERTYPE* const s = GetTeamPanelMercPortraitAt(x, y);
	if (!s) return FALSE;
...
BOOLEAN HandleTeamPanelMercPortraitLongPress(void)
{
	if (guiCurrentScreen != GAME_SCREEN) return FALSE;
	if (gsCurInterfacePanel != TEAM_PANEL) return FALSE;
	if (fInMapMode || gfInItemPickupMenu || gpItemPointer != NULL) return FALSE;
