...
{
	if (CheatSystem::noWeaponJam() && CheatSystem::appliesToPlayerSoldier(pSoldier))
	{
...
static void UseBlade(SOLDIERTYPE * const pSoldier, GridNo const sTargetGridNo)
{
	...
	if ( pTargetSoldier )
	{
		...
		if ( iDiceRoll <= iHitChance )
		{
			...
			if (CheatSystem::oneHitKill() &&
				CheatSystem::appliesToPlayerSoldier(pSoldier) &&
				CheatSystem::appliesToEnemyTarget(pTargetSoldier))
			{
				iImpact = pTargetSoldier->bLife;
			}
...
UINT32 CalcChanceToHitGun(SOLDIERTYPE *pSoldier, UINT16 sGridNo, UINT8 ubAimTime, UINT8 ubAimPos, BOOLEAN fModify )
{
	...
	if (CheatSystem::perfectHitChance() && CheatSystem::appliesToPlayerSoldier(pSoldier))
		return MAXCHANCETOHIT;
...
INT32 BulletImpact( SOLDIERTYPE *pFirer, SOLDIERTYPE * pTarget, UINT8 ubHitLocation, INT32 iOrigImpact, INT16 sHitBy, UINT8 * pubSpecial )
{
	...
	else if (CheatSystem::oneHitKill() &&
		CheatSystem::appliesToPlayerSoldier(pFirer) &&
		CheatSystem::appliesToEnemyTarget(pTarget))
	{
...
static UINT32 CalcChanceHTH(SOLDIERTYPE* pAttacker, SOLDIERTYPE* pDefender, UINT8 ubAimTime, UINT8 ubMode, bool skipSafetyCheck = false)
{
	...
	if (CheatSystem::perfectHitChance() && CheatSystem::appliesToPlayerSoldier(pAttacker))
		return MAXCHANCETOHIT;
	return (iChance);
}
