...
void EVENT_SoldierGotHit(SOLDIERTYPE* pSoldier, const UINT16 usWeaponIndex, INT16 sDamage, INT16 sBreathLoss, const UINT16 bDirection, const UINT16 sRange, SOLDIERTYPE* const att, const UINT8 ubSpecial, const UINT8 ubHitLocation, const INT16 sLocationGrid)
{
	...
	// God Mode: zero all damage for player soldiers
	if (CheatSystem::godMode() && CheatSystem::appliesToPlayerSoldier(pSoldier))
	{
		sDamage = 0;
		sBreathLoss = 0;
	}
...
UINT8 SoldierTakeDamage(SOLDIERTYPE* const pSoldier, INT16 sLifeDeduct, INT16 sBreathLoss, const UINT8 ubReason, SOLDIERTYPE* const attacker)
{
	...
	// Non-Lethal: clamp life to 1 for living player soldiers, preserving wounded states
	if (CheatSystem::nonLethalPlayerDamage() && CheatSystem::appliesToPlayerSoldier(pSoldier) && bOldLife > 0 && pSoldier->bLife <= 0)
	{
		pSoldier->bLife = 1;
	}
...
UINT32 SoldierDressWound( SOLDIERTYPE *pSoldier, SOLDIERTYPE *pVictim, INT16 sKitPts, INT16 sStatus )
{
	...
	if (uiActual > 0 && CheatSystem::fullMedicalHealing() &&
		CheatSystem::appliesToPlayerSoldier(pSoldier) &&
		pVictim->bTeam == OUR_TEAM && pVictim->bLife > 0)
	{
		pVictim->bLife = pVictim->bLifeMax;
		pVictim->bBleeding = 0;
		pVictim->bBreath = pVictim->bBreathMax;
		pVictim->sBreathRed = 0;
		DirtyMercPanelInterface(pVictim, DIRTYLEVEL2);
	}
