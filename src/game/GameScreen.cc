...
ScreenID MainGameScreenHandle(void)
{
	...
	// --- Tutorial system ---
	// Block everything while tutorial is showing
...
//	if( ( !gfTacticalDoHeliRun && !gfFirstHeliRun ) && ShouldTheHelpScreenComeUp( HELP_SCREEN_TACTICAL, FALSE ) )
	...
	{
		const bool shouldAutoShowTutorial = !gTutorial.fAutoShownThisSession && !gTutorial.fDontShowAgain && !gfTacticalPlacementGUIActive;
		EnterTacticalScreen( );
		showTutorialAfterFirstRender = shouldAutoShowTutorial && !gfTacticalPlacementGUIActive;
	...
	if (showTutorialAfterFirstRender)
	{
